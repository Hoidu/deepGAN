...
class DiscriminatorInfo(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size_classification, output_size_information):

        # call super class constructor
        super(DiscriminatorInfo, self).__init__()

        # init output size dimensionality
        self.output_size_classification = output_size_classification
        self.output_size_information = output_size_information
        ...
        self.map4_L = nn.Linear(hidden_size[2], output_size_classification + output_size_information, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.map4_L(x)
        a = torch.sigmoid(x[:, self.output_size_classification])
        b = x[:, self.output_size_classification:self.output_size_classification + self.output_size_information]
